...
export default function BookAClassClient({ sessions, initialClass }: { sessions: ClassSession[]; initialClass?: string }) {
  ...
  return (
    <div>
      ...
      {/* ── Session cards ── */}
      {filtered.length === 0 ? (
        <div className="py-6 sm:py-20 text-center">
          ...
          <p className="text-[#1a1a1a] font-medium mb-2" style={{ fontFamily: "var(--font-dm-sans), system-ui, sans-serif" }}>
            No upcoming sessions for{" "}<br className="sm:hidden" />{activeClass !== "All" ? <span className="text-[#006644] font-semibold">{activeClass}</span> : ""}
          </p>
      ...
      >
        ...
        >
          ...
          <div className="hidden sm:flex px-6 pt-4 pb-6 gap-6">
            ...
            <div className="flex flex-col gap-1 lg:flex-row lg:gap-5 text-sm text-[#6b7280] shrink-0 self-center mt-[4px]">
